...
By default the pages land in `docs/reference`, one directory per namespace plus index pages, with cross-page links rooted at `/reference` so the output drops straight into a static site generator or anything else that renders markdown. Each element page carries the widget's upstream documentation, its hierarchy, its children and slot rules, every prop with its type and default, every signal handler with its exact signature, and the methods reachable through `ref`. A `manifest.json` alongside the pages records the namespace and element lists, which is what you want for generating a sidebar.
