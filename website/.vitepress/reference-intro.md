...
Intrinsic elements such as `GtkButton` and `AdwApplicationWindow` are not listed here. Codegen produces them inside your project from the libraries you declare in `gtkx.config.ts` instead of publishing them to npm. The exact set depends on your configuration.
