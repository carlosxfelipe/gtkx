...
The documentation site lives in `website/` and uses VitePress. Run it from the repository root through turbo, not from inside `website/`: the API reference is generated by TypeDoc from each package's built `dist/`, so the site's build depends on the packages being built first.

```bash
turbo run build --filter=@gtkx/website # Build documentation
turbo run dev --filter=@gtkx/website   # Local preview
```
