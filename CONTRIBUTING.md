...
The documentation site lives in `website/` and uses VitePress. Run it from the repository root through turbo, not from inside `website/`. The API reference is generated by TypeDoc from each package's built `dist/`, so the build task depends on the packages being built first. The dev task does not build them, so run `pnpm build` before your first preview.

```bash
pnpm turbo run build --filter=@gtkx/website # Build documentation
pnpm turbo run dev --filter=@gtkx/website   # Local preview
```
