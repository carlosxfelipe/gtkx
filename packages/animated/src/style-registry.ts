...
import { camelToDash, getValueAsType, numberValueTypes, warnOnce } from "motion/react";
...
const declarationFor = (key: string, value: string | number): string | null => {
    ...
    if (key === "visibility") {
        return value === "hidden" ? "opacity: 0;" : null;
    }
    if (key === "transformOrigin") {
        return `transform-origin: ${String(value).split(" ").slice(0, 2).join(" ")};`;
    }
    const name = key.startsWith("--") ? key : camelToDash(key);
    return `${name}: ${getValueAsType(value, numberValueTypes[key])};`;
};
