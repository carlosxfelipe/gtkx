use native::ffi;
use native::types::IntegerKind;

#[test]
fn read_u8() {
    let value: u8 = 200;
    let ptr = &value as *const u8;
    let result = IntegerKind::U8.read_ptr(ptr);
    assert_eq!(result, 200.0);
}

#[test]
fn read_i8() {
    let value: i8 = -50;
    let ptr = &value as *const i8 as *const u8;
    let result = IntegerKind::I8.read_ptr(ptr);
    assert_eq!(result, -50.0);
}

#[test]
fn read_u16() {
    let value: u16 = 50000;
    let ptr = &value as *const u16 as *const u8;
    let result = IntegerKind::U16.read_ptr(ptr);
    assert_eq!(result, 50000.0);
}

#[test]
fn read_i16() {
    let value: i16 = -20000;
    let ptr = &value as *const i16 as *const u8;
    let result = IntegerKind::I16.read_ptr(ptr);
    assert_eq!(result, -20000.0);
}

#[test]
fn read_u32() {
    let value: u32 = 3_000_000_000;
    let ptr = &value as *const u32 as *const u8;
    let result = IntegerKind::U32.read_ptr(ptr);
    assert_eq!(result, 3_000_000_000.0);
}

#[test]
fn read_i32() {
    let value: i32 = -1_000_000_000;
    let ptr = &value as *const i32 as *const u8;
    let result = IntegerKind::I32.read_ptr(ptr);
    assert_eq!(result, -1_000_000_000.0);
}

#[test]
fn read_u64() {
    let value: u64 = 9_000_000_000;
    let ptr = &value as *const u64 as *const u8;
    let result = IntegerKind::U64.read_ptr(ptr);
    assert_eq!(result, 9_000_000_000.0);
}

#[test]
fn read_i64() {
    let value: i64 = -5_000_000_000;
    let ptr = &value as *const i64 as *const u8;
    let result = IntegerKind::I64.read_ptr(ptr);
    assert_eq!(result, -5_000_000_000.0);
}

#[test]
fn write_u8() {
    let mut value: u8 = 0;
    let ptr = &mut value as *mut u8;
    IntegerKind::U8.write_ptr(ptr, 123.0);
    assert_eq!(value, 123);
}

#[test]
fn write_i8() {
    let mut value: i8 = 0;
    let ptr = &mut value as *mut i8 as *mut u8;
    IntegerKind::I8.write_ptr(ptr, -42.0);
    assert_eq!(value, -42);
}

#[test]
fn write_u16() {
    let mut value: u16 = 0;
    let ptr = &mut value as *mut u16 as *mut u8;
    IntegerKind::U16.write_ptr(ptr, 12345.0);
    assert_eq!(value, 12345);
}

#[test]
fn write_i16() {
    let mut value: i16 = 0;
    let ptr = &mut value as *mut i16 as *mut u8;
    IntegerKind::I16.write_ptr(ptr, -12345.0);
    assert_eq!(value, -12345);
}

#[test]
fn write_u32() {
    let mut value: u32 = 0;
    let ptr = &mut value as *mut u32 as *mut u8;
    IntegerKind::U32.write_ptr(ptr, 1_234_567_890.0);
    assert_eq!(value, 1_234_567_890);
}

#[test]
fn write_i32() {
    let mut value: i32 = 0;
    let ptr = &mut value as *mut i32 as *mut u8;
    IntegerKind::I32.write_ptr(ptr, -1_234_567_890.0);
    assert_eq!(value, -1_234_567_890);
}

#[test]
fn write_u64() {
    let mut value: u64 = 0;
    let ptr = &mut value as *mut u64 as *mut u8;
    IntegerKind::U64.write_ptr(ptr, 9_876_543_210.0);
    assert_eq!(value, 9_876_543_210);
}

#[test]
fn write_i64() {
    let mut value: i64 = 0;
    let ptr = &mut value as *mut i64 as *mut u8;
    IntegerKind::I64.write_ptr(ptr, -9_876_543_210.0);
    assert_eq!(value, -9_876_543_210);
}

#[test]
fn to_ffi_value_u8() {
    let result = IntegerKind::U8.to_ffi_value(100.0);
    assert!(matches!(result, ffi::FfiValue::U8(100)));
}

#[test]
fn to_ffi_value_i8() {
    let result = IntegerKind::I8.to_ffi_value(-50.0);
    assert!(matches!(result, ffi::FfiValue::I8(-50)));
}

#[test]
fn to_ffi_value_u16() {
    let result = IntegerKind::U16.to_ffi_value(30000.0);
    assert!(matches!(result, ffi::FfiValue::U16(30000)));
}

#[test]
fn to_ffi_value_i16() {
    let result = IntegerKind::I16.to_ffi_value(-15000.0);
    assert!(matches!(result, ffi::FfiValue::I16(-15000)));
}

#[test]
fn to_ffi_value_u32() {
    let result = IntegerKind::U32.to_ffi_value(2_000_000_000.0);
    assert!(matches!(result, ffi::FfiValue::U32(2_000_000_000)));
}

#[test]
fn to_ffi_value_i32() {
    let result = IntegerKind::I32.to_ffi_value(-1_000_000_000.0);
    assert!(matches!(result, ffi::FfiValue::I32(-1_000_000_000)));
}

#[test]
fn to_ffi_value_u64() {
    let result = IntegerKind::U64.to_ffi_value(5_000_000_000.0);
    assert!(matches!(result, ffi::FfiValue::U64(5_000_000_000)));
}

#[test]
fn to_ffi_value_i64() {
    let result = IntegerKind::I64.to_ffi_value(-5_000_000_000.0);
    assert!(matches!(result, ffi::FfiValue::I64(-5_000_000_000)));
}

#[test]
fn to_stash_u8() {
    let values = [1.0, 2.0, 3.0];
    let stash = IntegerKind::U8.to_stash(&values);
    match stash.storage() {
        ffi::StashStorage::U8Vec(result) => assert_eq!(result, &vec![1u8, 2u8, 3u8]),
        _ => panic!("Expected U8Vec"),
    }
}

#[test]
fn to_stash_i32() {
    let values = [-100.0, 0.0, 100.0];
    let stash = IntegerKind::I32.to_stash(&values);
    match stash.storage() {
        ffi::StashStorage::I32Vec(result) => assert_eq!(result, &vec![-100i32, 0i32, 100i32]),
        _ => panic!("Expected I32Vec"),
    }
}

#[test]
fn vec_to_f64_u8() {
    let values: Vec<u8> = vec![10, 20, 30];
    let stash = ffi::Stash::from(values);
    let result = IntegerKind::U8.vec_to_f64(&stash).unwrap();
    assert_eq!(result, vec![10.0, 20.0, 30.0]);
}

#[test]
fn vec_to_f64_i32() {
    let values: Vec<i32> = vec![-100, 0, 100];
    let stash = ffi::Stash::from(values);
    let result = IntegerKind::I32.vec_to_f64(&stash).unwrap();
    assert_eq!(result, vec![-100.0, 0.0, 100.0]);
}

#[test]
fn vec_to_f64_wrong_type_fails() {
    let values: Vec<i32> = vec![1, 2, 3];
    let stash = ffi::Stash::from(values);
    let result = IntegerKind::U8.vec_to_f64(&stash);
    assert!(result.is_err());
}
