...
fn handle_read(object_id: ObjectId, type_: &Type, offset: usize) -> anyhow::Result<Value> {
    ...
    match type_ {
        Type::Integer(int_type) => {
            let number = match (int_type.size, int_type.sign) {
                (IntegerSize::_8, IntegerSign::Signed) => {
                    unsafe { *(field_ptr as *const i8) as f64 }
                }
                (IntegerSize::_8, IntegerSign::Unsigned) => {
                    unsafe { *(field_ptr as *const u8) as f64 }
                }
                (IntegerSize::_32, IntegerSign::Signed) => {
                    unsafe { *(field_ptr as *const i32) as f64 }
                }
                (IntegerSize::_32, IntegerSign::Unsigned) => {
                    unsafe { *(field_ptr as *const u32) as f64 }
                }
                (IntegerSize::_64, IntegerSign::Signed) => {
                    unsafe { *(field_ptr as *const i64) as f64 }
                }
                (IntegerSize::_64, IntegerSign::Unsigned) => {
                    unsafe { *(field_ptr as *const u64) as f64 }
                }
            };
        ...
        Type::GObject(_) => {
            ...
            let object = unsafe {
                glib::Object::from_glib_none(obj_ptr)
            };
