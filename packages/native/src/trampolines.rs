...
fn get_closure(user_data: *mut c_void) -> Option<*mut gobject_ffi::GClosure> {
...
unsafe extern "C" fn draw_func_trampoline(
    ...
    let Some(closure_ptr) = get_closure(user_data) else {
        return;
    };
...
///
/// This trampoline has special handling for nullable objects and uses
/// a different argument conversion pattern, so it's implemented manually.
///
/// # Safety
///
